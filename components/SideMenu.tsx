    ...
    >
      ...
      <div ref={sideBarRef} className="min-w-72 max-w-96 bg-black h-screen p-10  border-r border-r-shop_light_green flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between gap-5">
          ...
          <button  
            onClick={onClose}
            className="hover:text-shop_light_green hover:cursor-pointer hoverEffect "
          >
            <X />
          </button>
        ...
        <div className="flex flex-col space-y-3.5 font-semibold tracking-wide">
            {headerData?.map((item) => (
                ...
                href={item?.href} 
                key={item?.title}
                className={`hover:text-shop_light_green hoverEffect ${pathname === item?.href ? 'text-shop_light_green' : 'text-white'}`}>
                    {item?.title}
                </Link>
            ))}
        </div> 
